...
function EntryCard({ entry, onDelete, onEdit }: { entry: JournalEntry; onDelete: () => void; onEdit: () => void }) {
  ...
  return (
    <TouchableOpacity style={styles.entryCard} onPress={() => setExpanded(e => !e)} activeOpacity={0.8} accessibilityRole="button" accessibilityHint="Double tap to expand">
...
export default function JournalScreen() {
  ...
  const filtered = search
    ? data.filter(e => e.content.toLowerCase().includes(search.toLowerCase()))
    : data
  ...
  const ListHeader = (
    <View style={styles.pageHeader}>
      ...
      <View style={styles.headerRow}>
        ...
        <TouchableOpacity style={[styles.addBtn, isOffline && styles.btnDisabled]} onPress={openCreate} disabled={isOffline} accessibilityRole="button" accessibilityLabel="Add new journal entry">
          <Text style={styles.addBtnText}>+ New</Text>
        </TouchableOpacity>
      </View>
      <SearchBar value={search} onChangeText={setSearch} placeholder="Search journal..." />
    </View>
  )
  ...
  return (
    <ScreenWrapper scroll={false} padHorizontal={false}>
      ...
      >
        ...
        <View style={styles.toggle}>
          {(['morning', 'evening'] as const).map(p => (
            <TouchableOpacity key={p} style={[styles.toggleBtn, form.period === p && styles.toggleActive]} onPress={() => set('period', p)} accessibilityRole="tab" accessibilityState={{ selected: form.period === p }}>
        ...
        <View style={styles.moodRow}>
          {[1, 2, 3, 4, 5].map(m => (
            <TouchableOpacity key={m} style={[styles.moodBtn, form.mood === m && { borderColor: moodColor(m), backgroundColor: colors.bgDeep }]} onPress={() => set('mood', m)} accessibilityRole="button" accessibilityState={{ selected: form.mood === m }} accessibilityLabel={`Mood ${m} of 5`}>
              <Text style={{ fontSize: 22 }}>{moodEmoji(m)}</Text>
            </TouchableOpacity>
          ))}
        </View>
        <FormField label="Entry *" value={form.content} onChangeText={t => set('content', t)} placeholder="Write your thoughts..." multiline numberOfLines={6} style={{ height: 140 }} maxLength={2000} />
        <FormField label="Gratitude" optional value={form.gratitude} onChangeText={t => set('gratitude', t)} placeholder="What are you grateful for?" multiline numberOfLines={3} maxLength={2000} />
        <FormField label="Highlights" optional value={form.highlights} onChangeText={t => set('highlights', t)} placeholder="Best moments of the day..." multiline numberOfLines={3} maxLength={2000} />
      </ModalForm>
      ...
      <FlatList
        ...
        renderItem={({ item }) => (
          <EntryCard entry={item} onDelete={() => handleDelete(item.id)} onEdit={() => openEdit(item)} />
        )}
...
const styles = StyleSheet.create({
  ...
  entryCard: { backgroundColor: colors.bgCard, borderRadius: radius.lg, borderWidth: 1, borderColor: colors.border, padding: spacing.lg, gap: spacing.sm },
